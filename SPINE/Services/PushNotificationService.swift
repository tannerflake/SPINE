...
enum SpineDeepLink {
    /// The app's canonical URL scheme. `wellread` is the original scheme and is
    /// still accepted forever: club invites and push payloads already sent to
    /// readers use it. New links are minted with `spine`.
    static let scheme = "spine"
    static let legacyScheme = "wellread"

    /// True for either the current `spine://` scheme or the legacy `spine://` one.
    static func isAppScheme(_ url: URL) -> Bool {
        guard let s = url.scheme?.lowercased() else { return false }
        return s == scheme || s == legacyScheme
    }
    /// `spine://club/join/{CODE}` — a club invite link; opens the join sheet
    /// with the code filled in (or joins outright once signed in).
    static func clubInviteCode(from url: URL) -> String? {
        guard isAppScheme(url) else { return nil }
    ...
    /// `spine://club/{clubId}` — straight to a club the reader belongs to.
    ...
    /// `spine://post/{postUUID}` — opens Feed and the post’s comment thread.
    ...
    /// `spine://queue` — a widget tap on one of your own reading-now covers.
    static func isQueueLink(_ url: URL) -> Bool {
        isAppScheme(url) && url.host?.lowercased() == "queue"
    }

    /// `spine://book/{bookId}?reader={uid}` — a widget tap on a friend's cover.
    ...
    static func bookLink(from url: URL) -> (bookId: String, readerUid: String?)? {
        guard isAppScheme(url), url.host?.lowercased() == "book" else { return nil }
...
enum PushNotificationService {
        ...
        center.getDeliveredNotifications { delivered in
            let ids = delivered
                .filter { note in
                    ...
                    return SpineDeepLink.pushNotificationType(from: info) == "blend_request"
    ...
    private static func routeRemoteNotificationTap(userInfo: [AnyHashable: Any]) {
        ...
        if type == "friend_review_posted" || type == "review_liked" || type == "review_mentioned" {
            guard let postId = SpineDeepLink.postId(fromNotificationUserInfo: userInfo) else {
        ...
        guard let postId = SpineDeepLink.postId(fromNotificationUserInfo: userInfo) else { return }
        openPostComments(postId: postId, commentId: SpineDeepLink.commentId(fromNotificationUserInfo: userInfo))
    }
...
extension Notification.Name {
    ...
    static let spineOpenFeed = Notification.Name("spineOpenFeed")
    ...
    static let spineOpenFeedPost = Notification.Name("spineOpenFeedPost")

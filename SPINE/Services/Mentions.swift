...
enum PeopleSimilarity {
    ...
    /// People shared between you and a reader you don't follow: the count shown
    /// under their name in the feed's follow suggestions.
    ///
    /// A mutual is a *person*, so the two directions are unioned rather than
    /// added — someone you both follow who also follows the candidate counts
    /// once, not twice. Unlike `score` this drops the "they already follow you"
    /// bonus, which isn't a shared person. The founder and you are excluded:
    /// the founder is connected to the whole roster, and you are a mutual of
    /// everyone by definition.
    static func mutualCount(
        candidateUid: String,
        candidateFollowing: [String],
        following: Set<String>,
        peers: [(uid: String, following: [String])],
        ...
        let excluded = Set([SpineFounder.uid] + [currentUid].compactMap { $0 })
        let pool = following.subtracting(excluded)
        var mutuals = pool.intersection(candidateFollowing)
        for peer in peers where !excluded.contains(peer.uid) && peer.following.contains(candidateUid) {
            mutuals.insert(peer.uid)
        }

...
struct BookClub: Identifiable, Equatable {
    ...
    var inviteURL: URL {
        URL(string: "\(SpineDeepLink.scheme)://club/join/\(inviteCode)")!
    }
...
extension BookClub {
    ...
    /// Second fixture for `-uiPreviewClubsMulti`, so the club list (rather than
    /// the solo-club shortcut) can be previewed.
    static let uiPreviewDemoSecond: BookClub = {
        let now = Date()
        let base = BookClub.uiPreviewDemo
        let pick = Pick(
            id: "pick-demo-2",
            bookId: "r3",
            title: "Piranesi",
            ...
            coverURL: "",
            pageCount: 245,
            chosenAt: now.addingTimeInterval(-86400 * 3),
            chosenBy: "demo-hannah",
            meetingAt: Calendar.current.date(bySettingHour: 18, minute: 30, second: 0, of: now.addingTimeInterval(86400 * 5)) ?? now.addingTimeInterval(86400 * 5)
        )
        return BookClub(
            id: "club-demo-2",
            name: "Thursday Night Reads",
            createdBy: "demo-hannah",
            createdAt: now.addingTimeInterval(-86400 * 30),
            updatedAt: now,
            updatedBy: "demo-hannah",
            memberIds: ["ui-preview", "demo-hannah", "demo-marcus"],
            adminIds: ["demo-hannah"],
            everyoneIsAdmin: true,
            members: [
                "ui-preview": base.members["ui-preview"]!,
                "demo-hannah": base.members["demo-hannah"]!,
                "demo-marcus": base.members["demo-marcus"]!,
            ],
            inviteCode: "R2WTN9",
            currentPick: pick,
            pastPicks: []

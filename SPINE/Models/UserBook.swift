...
/// sorts to the bottom of every timeline, and reads back as the same phrase the
/// picker offered, "A long, long time ago", wherever a finish date is shown.
enum ReadDate {
    ...
    /// Label shown in place of a formatted date for long-ago reads. Always the
    /// full phrase the picker chip uses: a bare "Old" reads as nonsense in the
    /// sentences these land in ("read Old").
    static let longAgoLabel = "A long, long time ago"

    /// Same phrase mid-sentence, where it follows a lowercase word ("read ...").
    static let longAgoLabelInline = "a long, long time ago"
    ...
    /// "A long, long time ago" for long-ago reads, otherwise the formatted date.
    /// Pass `capitalized: false` when the label continues a sentence.
    static func label(_ date: Date, formatter: DateFormatter, capitalized: Bool = true) -> String {
        guard isLongAgo(date) else { return formatter.string(from: date) }
        return capitalized ? longAgoLabel : longAgoLabelInline
    }

    /// "A long, long time ago" instead of "1900" in year sections and year filters.
    static func yearLabel(_ year: Int) -> String {
        isLongAgo(year: year) ? longAgoLabel : String(year)
    }
}

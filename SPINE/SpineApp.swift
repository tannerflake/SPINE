...
class AppDelegate: NSObject, UIApplicationDelegate, MessagingDelegate, UNUserNotificationCenterDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        if SpineDeepLink.pushNotificationType(from: userInfo) == "blend_request_withdrawn",
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplication.OpenURLOptionsKey: Any] = [:]) -> Bool {
        if SpineDeepLink.isAppScheme(url), url.host == "goodreads-import" {
            NotificationCenter.default.post(name: .openGoodreadsImport, object: nil)
            return true
        }
        if SpineDeepLink.isAppScheme(url), url.host == "link-import" {
            NotificationCenter.default.post(name: .openLinkImport, object: nil)
            return true
        }
        if let postId = SpineDeepLink.postId(from: url) {
            ...
            PushNotificationService.routeAfterClearingPresentedModals {
                NotificationCenter.default.post(name: .spineOpenFeedPost, object: nil, userInfo: ["postId": postId])
            }
            return true
        }
        // Widget taps.
        if SpineDeepLink.isQueueLink(url) {
            PushNotificationService.routeToQueue()
            return true
        }
        if let link = SpineDeepLink.bookLink(from: url) {
    ...
    var body: some Scene {
        WindowGroup {
            RootView()
                ...
                .onOpenURL { url in
                    // Club invite link: `spine://club/join/{CODE}`.
                    if let code = SpineDeepLink.clubInviteCode(from: url) {
                    ...
                    if let clubId = SpineDeepLink.clubId(from: url) {
                    ...
                    } else if SpineDeepLink.isAppScheme(url), url.host == "link-import" {
                        handleLinkImportFromShare()
                    } else if SpineDeepLink.isQueueLink(url) {
                        // Widget: tapped one of your own reading-now covers.
                        PushNotificationService.routeToQueue()
                    } else if let link = SpineDeepLink.bookLink(from: url) {
                        // Widget: tapped a cover on someone else's shelf.
                        PushNotificationService.routeToBookProfile(bookId: link.bookId, readerUid: link.readerUid)
                    } else if let postId = SpineDeepLink.postId(from: url) {

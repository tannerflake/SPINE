...
final class CoverImageCache {
    ...
    private let diskQueue = DispatchQueue(label: "com.spine.covercache.disk")
    ...
    private func diskCacheDirectory() -> URL? {
        ...
        // On-disk cache path. Keeps the old "WellRead" name so the SPINE rename
        // does not orphan every reader's cover cache.

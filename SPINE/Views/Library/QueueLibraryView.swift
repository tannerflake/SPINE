...
/// Someone else's queue has no drag targets, so the gaps between covers are plain spacers.
/// The cover-size math below must use this same value, or the row is sized for 16pt gaps it
/// never draws and the shelf ends short of the trailing edge.
private let queueReadOnlySlotWidth: CGFloat = 6
...
private struct QueueShelfDropSlot: View {
    ...
    var body: some View {
        Group {
            if readOnly {
                Color.clear
                    .frame(minWidth: queueReadOnlySlotWidth, maxWidth: fillsRow ? .infinity : queueReadOnlySlotWidth)
...
private struct QueueSectionGrid: View {
    ...
    var body: some View {
        ...
        let slotWidth = readOnly ? queueReadOnlySlotWidth : queueDropSlotWidth
        let narrowSlotCount = queueBooksPerRow + 1
        let slotSpace = CGFloat(narrowSlotCount) * slotWidth
        // Cap only guards absurd covers on very wide layouts; on phone widths the row fills edge to edge.
        let bookSize = min(CGFloat(120), max(CGFloat(56), (available - slotSpace) / CGFloat(queueBooksPerRow)))

...
//  small overline labels. Action bar below uses flat capsule buttons (SpineButtonStyle).
//
...
struct BookProfileView: View {
    ...
    /// Match-branded card: same target + ink-ramp imagery as the hero badge,
    ...
    private func reviewFooter(ub: UserBook) -> some View {
        ...
        return (
            ...
            + Text(dates.map { ReadDate.label($0, formatter: Self.readDateFormatter, capitalized: false) }.joined(separator: " \u{00B7} "))
    ...
    private var refresherWindow: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Button {
                ...
                Label("Refresh my memory", systemImage: "brain.head.profile")
            }
            .buttonStyle(.spine(.primary, size: .regular))
        }
    ...
    private var readByAvatarFan: some View {
        ...
        return Button {
            ...
            // SwiftUI hit-tests the drawn circles otherwise, leaving the gaps
            // between fanned avatars dead.
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
    }
    ...
    private func scrollToReadBy() {
        guard let pageScrollProxy, !readByReaders.isEmpty else { return }
        withAnimation(.easeInOut(duration: 0.35)) {
            pageScrollProxy.scrollTo(Self.readBySectionAnchor, anchor: .top)
        }
    }
    ...
    private var actionButtonRow: some View {
        ...
        HStack(spacing: 10) {
            if onNotInterested != nil {
                ...
                // Fixed width so it reads as "the exit," not a fourth peer
                // action alongside the shelf buttons.
            ...
            if showReadingNowDNF {
                ...
                .buttonStyle(.spine(.destructive, size: .regular))
            }
            if onConfirmRead != nil {
                Button(action: {
                    ...
                    actionLabel(isCurrentlyReading ? "FINISHED!" : "FINISHED")
                }
                // FINISHED is the main action once the book is being read (or
                // QUEUE has stepped aside for a shelf CTA); otherwise it's a
                // peer of READING and QUEUE carries the weight.
                .buttonStyle(.spine(finishedIsPrimary ? .primary : .secondary, size: .regular))
            }
            if !showReadingNowDNF, onWantToRead != nil || onRemoveFromQueue != nil {
                Group {
                    if isInQueue && onRemoveFromQueue != nil {
                        ...
                        // outline so there's a single clear main button.
                        .buttonStyle(.spine(showShelfAction ? .secondary : .primary, size: .regular))
                    }
                }
            }
        }
    ...
    /// FINISHED carries the weight when the book is mid-read, and when QUEUE
    /// isn't on offer as the main action (already queued, or a shelf CTA is
    /// showing above the row).
    private var finishedIsPrimary: Bool {
        if isCurrentlyReading || showReadingNowDNF { return true }
        if showShelfAction { return false }
        return isInQueue || onWantToRead == nil
    }

    /// Uppercase tracked label for the shelf action row. Fill, outline, and
    /// press state come from `SpineButtonStyle` at the call site.

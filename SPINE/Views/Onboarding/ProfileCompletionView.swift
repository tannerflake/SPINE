...
struct ProfileCompletionView: View {
            ...
            HStack(alignment: .center, spacing: 16) {
                ZStack {
                    if let user = profileUserForAvatar {
                        ...
                        UserAvatarView(urlString: nil, displayName: nil, size: 80)
                    }
                    if isUploadingPhoto {
    ...
    private var canFinishOnboarding: Bool {
        SpineTagCatalog.shared.whitelist(Array(selectedInterestTags)).count >= 2
    }
    ...
    private func submitEditProfile() {
        ...
        Task {
            do {
                let tags = SpineTagCatalog.shared.whitelist(Array(selectedInterestTags))

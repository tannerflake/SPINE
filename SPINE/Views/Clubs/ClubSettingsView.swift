...
//  SPINE
//
//  Rename, governance, admins, member management, leave, delete. When the club
//  page is the whole tab (their only club), this is also where starting or
//  joining another one lives.
//
...
struct ClubSettingsView: View {
    ...
    /// pull on the top strip, which is the one place that does dismiss.
    ...
    var body: some View {
                    ...
                    Section {
                        if let onStartClub {
                            Button {
                                ...
                                Label("Start another club", systemImage: "plus")
                                    .foregroundStyle(Theme.textPrimary)
                            }
                        }
                        if let onJoinClub {
                        ...
                        Text("Be in as many as you want. Once you're in two, the Clubs tab lists them all.")
                    }
                }
        ...
        .offset(y: dragOffset)
        .overlay(alignment: .topLeading) { dismissStrip }
        .presentationBackground(Theme.background)
        .presentationDragIndicator(.visible)
        .interactiveDismissDisabled(true)
    }

    /// The grabber and title row: dragging here pulls the sheet down, anywhere
    /// else is the list's to scroll. Stops short of the Done button.
            ...
            .gesture(
                DragGesture(minimumDistance: 6)
                    ...
                    .onEnded { value in
                        ...
                        withAnimation(.spring(response: 0.32, dampingFraction: 0.85)) { dragOffset = 0 }
                    }
            )
            // Outside the gesture's own shape, so Done stays tappable.

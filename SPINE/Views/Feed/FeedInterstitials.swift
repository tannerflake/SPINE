...
//  laid out as a horizontal carousel the reader swipes through (a dozen cards,
//  the fourth peeking in from the trailing edge so the swipe is discoverable),
//  each with a quick Follow and an X that hides that reader from suggestions
//  for good. Both rows hold their content until the feed is refreshed, and
//  even then only the rows the reader actually reached are re-picked.
//
...
final class FeedInterstitialModel: ObservableObject {
    ...
    /// Covers per "Selected for you" row: three plus the "Discover more" tile.
    static let itemsPerSlot = 3
    /// Cards per "Readers to follow" carousel. Enough to swipe through several
    /// screens without having to X anyone out to see who's next; the ranked
    /// roster is capped well above this so a full row is the norm.
    static let readersPerSlot = 12
    ...
    func readers(for slot: Int, ranked: [PeopleStripModel.Reader], following: Set<String>, persistedDismissed: [String]) -> [PeopleStripModel.Reader] {
        ...
        if ids.count < Self.readersPerSlot {
            ...
            for reader in eligible where !taken.contains(reader.uid) {
                ...
                if ids.count == Self.readersPerSlot { break }
            }
        }
...
/// Inner gutter between an interstitial chunk's edge and its content — the same
/// step a post's text column keeps off its tier pillar.
let feedInterstitialContentInset: CGFloat = 14
/// Total inset of interstitial content from the screen edge (the chunk's own
/// inset plus that gutter). Cell widths are sized off this.
let feedInterstitialScreenInset: CGFloat = feedRowInset + feedInterstitialContentInset

extension View {
    /// Wraps an interstitial row in the chunk the feed's posts use: the same
    /// surface tint, card radius and row inset. The rows used to sit bare on the
    /// page between tinted chunks, which read as a gap rather than as a row.
    func feedInterstitialChunk() -> some View {
        self
            .padding(.vertical, 14)
            ...
            // A post carries its own lead-in through its header's top padding.
            // An interstitial has no header, so without this it sits 8pt under
            // the chunk above it and 18pt over the one below.
...
struct FeedBookPicksRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            FeedInterstitialHeader(icon: "safari.fill", title: "SELECTED FOR YOU")
            ...
            .padding(.horizontal, feedInterstitialContentInset)
        }
        .feedInterstitialChunk()
    ...
    private var seeMoreCell: some View {
        Button {
            ...
            .background(
                // Paper-on-paper: the row's chunk is surface-tinted already.
    ...
    var body: some View {
        Group {
            if isLoading {
                ...
                shape.fill(Theme.surfaceElevated.opacity(0.7))
            }
        }
...
/// Not-yet-followed readers as cards in a horizontal carousel: avatar, name,
/// Follow. Three cards fit the screen with the fourth peeking in from the
/// trailing edge, so swiping right-to-left reveals more readers without having
/// to X anyone out. The X top-right hides that reader from future suggestions.
struct FeedPeoplePicksRow: View {
    ...
    /// Cards fully visible before the carousel scrolls.
    private static let visibleCards: CGFloat = 3
    /// How much of the next card shows past the trailing edge: the cue that
    /// the row swipes.
    private static let nextCardPeek: CGFloat = 28
    /// Widest a card goes on a wide screen (iPad), so it stays a card and not
    /// a banner.
    private static let maxCardWidth: CGFloat = 132
    /// Two lines of the 12pt name label.
    private static let nameBlockHeight: CGFloat = 30
    /// One line of the 11pt mutuals label. Always reserved, so a reader with no
    /// mutuals doesn't ride a shorter card than their neighbours.
    private static let mutualsBlockHeight: CGFloat = 14

    /// Sized from the screen, like the picks row's covers, so the row lays out
    /// at its final width before any reader has loaded.
    private var cardWidth: CGFloat {
        let available = UIScreen.main.bounds.width
            - feedInterstitialScreenInset * 2
            - Self.cardSpacing * Self.visibleCards
            - Self.nextCardPeek
        return min((available / Self.visibleCards).rounded(.down), Self.maxCardWidth)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            .contentMargins(.horizontal, feedInterstitialContentInset, for: .scrollContent)
    ...
    private func readerCard(_ reader: PeopleStripModel.Reader) -> some View {
                ...
                VStack(spacing: 8) {
                    ...
                    VStack(spacing: 1) {
                        Text(reader.user.displayName)
                            ...
                            .lineLimit(2)
                            .multilineTextAlignment(.center)
                            // Always reserve two lines so one-line names don't
                            // make a shorter card than their neighbours in the
                            // carousel, which would leave the Follow buttons at
                            // uneven heights across the row.
                            .frame(minHeight: Self.nameBlockHeight, alignment: .top)
                        mutualsLabel(reader)
                    }
                }
        ...
        .frame(maxWidth: .infinity)
        .background(
            // Paper-on-paper, like a post's inline comment card: the row's own
            // chunk is already surface-tinted, so a matching fill would swallow
            // the cards.
    ...
    /// "3 mutuals" under the name — readers you and they both know. Nobody in
    /// common leaves the line blank rather than reading "0 mutuals"; the height
    /// is held either way so the Follow buttons stay level across the row.
    @ViewBuilder
    private func mutualsLabel(_ reader: PeopleStripModel.Reader) -> some View {
        Group {
            if reader.mutualCount > 0 {
                Text(reader.mutualCount == 1 ? "1 mutual" : "\(reader.mutualCount) mutuals")
        ...
        .frame(height: Self.mutualsBlockHeight)
    }

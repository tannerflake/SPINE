...
final class PeopleStripModel: ObservableObject {
    ...
    struct Reader: Identifiable, Equatable {
        ...
        let score: Int
        /// People shared with the signed-in member (see
    ...
    private func seedUIPreviewRoster() {
        ...
        discoverable = [
            ...
            reader("pv-4", "Theo Brandt", books: 31, score: 1, mutuals: 1),
            // No mutuals: the card drops the line but keeps its height.
            reader("pv-5", "Wren Okafor", books: 23, score: 0),
            reader("pv-6", "Sam Idowu", books: 12, score: 0),
            reader("pv-7", "Noor Haddad", books: 88, score: 0),
    ...
    /// Shared people between you and a not-yet-followed reader — the "N mutuals"
    /// line on the feed's follow-suggestion cards.
    private func mutualCount(
        candidateUid: String,
        candidateFollowing: [String],
        peers: [(uid: String, user: User)]
    ) -> Int {
        PeopleSimilarity.mutualCount(
            candidateUid: candidateUid,
            candidateFollowing: candidateFollowing,
            following: followingSet,
            peers: peers.map { (uid: $0.uid, following: $0.user.following) },
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(alignment: .top, spacing: Self.peopleCellSpacing) {
                    ForEach(0..<6, id: \.self) { _ in
                        VStack(spacing: 8) {
                            ShimmerCircle(size: Self.avatarSize)
    ...
    private func readerCell(reader: PeopleStripModel.Reader, isFollowed: Bool) -> some View {
        ...
        return NavigationLink(value: reader.uid) {
            VStack(spacing: 8) {
                ...
                circleAvatar(user: reader.user, size: Self.avatarSize)
                    ...
                    .overlay(alignment: .bottomLeading) {
                        if !readingNow.isEmpty {
                            ReadingNowFanStack(books: readingNow, coverWidth: 22)
                                .offset(x: -10, y: 9)
                        }
                    }
                ...
                // First name only, like a story row; the full name is one tap away.
                Text(reader.user.firstNameLabel)

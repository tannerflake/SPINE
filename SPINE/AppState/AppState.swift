...
final class AppState: ObservableObject {
    ...
    /// Set when opening a feed post from a push or `spine://` URL; Feed opens comments when resolved.
    ...
    private static let libraryCacheSaveQueue = DispatchQueue(label: "com.spine.library-cache-save", qos: .utility)
    ...
    /// Goes through `deleteReadReview` so the read dates, the review, and any
    /// finished-book / hidden discussion posts go with it — a bare userBook
    /// delete used to leave the feed post behind as an orphan.
    func removeFromReadList(book: Book) {
        guard currentUserId != nil else { return }
        guard let userBook = userBooks.first(where: { $0.bookId == book.id && $0.status == .read }) else { return }
        Task { @MainActor in
            // Leave the shelf immediately; a failed delete comes back on the listener echo.
            withAnimation(.easeInOut(duration: 0.3)) {
                userBooks.removeAll { $0.id == userBook.id }
            }
            _ = await deleteReadReview(userBook: userBook)
        }
    }
